...
static inline void handle_interrupts(timer16_Sequence_t timer, volatile uint16_t *TCNTn, volatile uint16_t* OCRnA){
 ...
 if( SERVO_INDEX(timer,Channel[timer]) < ServoCount && Channel[timer] < SERVOS_PER_TIMER) {
    ...
    //### Extension for slowmove
		//### Increment ticks by speed until we reach the target, when the target is reached, speed is set to 0 to disable that code.
...
uint8_t ServoLib::attach(int pin) { return this->attach(pin, MIN_PULSE_WIDTH, MAX_PULSE_WIDTH); }

uint8_t ServoLib::attach(int pin, int min, int max) {
...
void ServoLib::detach(){
 servos[this->servoIndex].Pin.isActive = false;
 timer16_Sequence_t timer = SERVO_INDEX_TO_TIMER(servoIndex);
 if(isTimerActive(timer) == false) { finISR(timer); }
}

void ServoLib::write(int value){
 byte channel = this->servoIndex;
 servos[channel].value = value;
 if(value < MIN_PULSE_WIDTH){ //### treat values less than 544 as angles in degrees (valid values in microseconds are handled as microseconds) updated to use constrain() instead of if(), pva
...
void ServoLib::writeMicroseconds(int value){
...
void ServoLib::write(int value, uint8_t speed) {
//### speed = 0 - Full speed, identical to write
//### speed = 1 - Minimum speed & speed = 255 - Maximum speed
 byte channel = this->servoIndex;
 servos[channel].value = value;
	if (speed) {
...
void ServoLib::write(int value, uint8_t speed, bool wait) {
 write(value, speed);
...
void ServoLib::stop() { write(read()); }

void ServoLib::slowmove(int value, uint8_t speed) { write(value, speed); }
//### End of Extension for slowmove

int ServoLib::read() //### return the value as degrees
{
 return map( this->readMicroseconds()+1, SERVO_MIN(), SERVO_MAX(), 0, 180);
}

int ServoLib::readMicroseconds()
{
...
bool ServoLib::attached()
{
 return servos[this->servoIndex].Pin.isActive ;
}

uint8_t ServoLib::sequencePlay(servoSequencePoint sequenceIn[], uint8_t numPositions, bool loop, uint8_t startPos) {
...
uint8_t ServoLib::sequencePlay(servoSequencePoint sequenceIn[], uint8_t numPositions) {
 return sequencePlay(sequenceIn, numPositions, true, 0);
}

void ServoLib::sequenceStop() {
 write(read());
 this->curSeqPosition = CURRENT_SEQUENCE_STOP;
}
